...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "INSERT INTO teachers (username, firstName, lastName, contactNumber, teacherAddress, passcode, userType) 
                  VALUES ('$newUsername', '$newFirstName', '$newLastName', '$newContactNumber', '$newAddress', '$newPassword', 'teacher')";
        $conn->query($query);
    }
} 
                    ...
                    <form method="POST" action="" id="newAddForm">
                        <div style="width:800px;">
                            ...
                            <div class="col-auto" style="display:flex; justify-content: flex-end; margin-bottom:10px;">
                                <button type="submit" class="btn btn-success p-2" style="margin-right:10px;">ADD Teacher</button>
    ...
    <script>
        document.getElementById('newAddForm').addEventListener('submit', function(event) {
            ...
            Swal.fire({
                ...
                confirmButtonText: 'Yes, Add Teacher',
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    Swal.fire({
                        title: "Teacher Added!",
                        text: "The Teacher has been Added!",
                        icon: "success",
                    })
                    .then(() => {
                        this.submit();
